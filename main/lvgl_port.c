    ...
    if (disp_refr->inv_area_joined[i] == 0) {
      flush_ver =
        (disp_refr->inv_areas[i].y2 + 1 -
         disp_refr->inv_areas[i].y1); // Calculate vertical flush size
      flush_hor =
        (disp_refr->inv_areas[i].x2 + 1 -
         disp_refr->inv_areas[i].x1); // Calculate horizontal flush size
      break; // Exit the loop after finding the first unjoined area
    }
  }
  /* Check if the current full screen refreshes */
  cur_status = ((flush_ver == drv->ver_res) && (flush_hor == drv->hor_res)) ? (FLUSH_STATUS_FULL)
                                                                            : (FLUSH_STATUS_PART);
...
static void flush_dirty_copy(void *dst, void *src, lv_port_dirty_area_t *dirty_area) {
  ...
  for (int i = 0; i < dirty_area->inv_p; i++) {
    ...
    if (dirty_area->inv_area_joined[i] == 0) {
      ...
      rotate_copy_pixel(
        src, dst, x_start, y_start, x_end, y_end, LV_HOR_RES, LV_VER_RES, LVGL_PORT_ROTATION_DEGREE
      );
    }
  }
}
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map) {
  esp_lcd_panel_handle_t panel_handle =
    (esp_lcd_panel_handle_t)drv->user_data; // Get the panel handle from driver user data
  ...
  if (lv_disp_flush_is_last(drv)) {
    ...
    if (drv->full_refresh) {
      ...
      esp_lcd_panel_draw_bitmap(
        panel_handle, offsetx1, offsety1, offsetx2 + 1, offsety2 + 1, next_fb
      );
...
static lv_disp_t *display_init(esp_lcd_panel_handle_t panel_handle) {
  ...
  #if (LVGL_PORT_LCD_RGB_BUFFER_NUMS == 3) && (LVGL_PORT_ROTATION_DEGREE == 0) && \
    LVGL_PORT_FULL_REFRESH
  // With three buffers and full-refresh, one buffer is always available for rendering
  ESP_ERROR_CHECK(
    esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 3, &lvgl_port_rgb_last_buf, &buf1, &buf2)
  );
  ...
  buf1 = heap_caps_malloc(
    buffer_size * sizeof(lv_color_t), LVGL_PORT_BUFFER_MALLOC_CAPS
  ); // Allocate memory
  assert(buf1); // Ensure allocation succeeded
  ESP_LOGI(
    TAG, "LVGL buffer size: %dKB", buffer_size * sizeof(lv_color_t) / 1024
  ); // Log buffer size
#endif /* LVGL_PORT_AVOID_TEAR_ENABLE */
...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {
  esp_lcd_touch_handle_t tp =
    (esp_lcd_touch_handle_t)indev_drv->user_data; // Get touchpad handle from user data
...
esp_err_t lvgl_port_init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle) {
  ...
  BaseType_t core_id = (LVGL_PORT_TASK_CORE < 0)
    ? tskNO_AFFINITY
    : LVGL_PORT_TASK_CORE; // Determine core ID for the task
...
bool lvgl_port_lock(int timeout_ms) {
  ...
  const TickType_t timeout_ticks =
    (timeout_ms < 0) ? portMAX_DELAY : pdMS_TO_TICKS(timeout_ms); // Convert timeout to ticks
