...
static TaskHandle_t lvgl_task_handle = NULL; // Handle for the LVGL task
  ...
  #if LVGL_PORT_FULL_REFRESH && LVGL_PORT_LCD_RGB_BUFFER_NUMS == 2
...
static lv_disp_t *display_init(esp_lcd_panel_handle_t panel_handle) {
...
#if LVGL_PORT_AVOID_TEAR_ENABLE
  // To avoid tearing effect, at least two frame buffers are needed: one for
  // LVGL rendering and another for RGB output
  buffer_size = LVGL_PORT_H_RES * LVGL_PORT_V_RES;
  #if (LVGL_PORT_LCD_RGB_BUFFER_NUMS == 3) && LVGL_PORT_FULL_REFRESH
  // With three buffers and full-refresh, one buffer is always available for
  // rendering
  ESP_ERROR_CHECK(
  ...
  lv_disp_draw_buf_init(&disp_buf, buf1, buf2,
                        buffer_size); // Initialize the draw buffer
...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {
  ...
  if (touchpad_pressed && touchpad_cnt > 0) {
    ...
    ESP_LOGD(TAG, "Touch position: %d,%d", touchpad_x,
             touchpad_y); // Log touch position
  } else {
...
static esp_err_t tick_init(void) {
  ...
  ESP_ERROR_CHECK(esp_timer_create(&lvgl_tick_timer_args,
                                   &lvgl_tick_timer)); // Create the timer
...
esp_err_t lvgl_port_init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle) {
  ...
  if (ret != pdPASS) {
    ESP_LOGE(TAG,
             "Failed to create LVGL task"); // Log error if task creation fails
