...
static struct peer_svc *peer_svc_find(
    struct peer *peer, uint16_t svc_start_handle, struct peer_svc **out_prev
);
...
#if MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) || MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES)
static int peer_inc_disced(
    uint16_t conn_handle,
    const struct ble_gatt_error *error,
    const struct ble_gatt_incl_svc *service,
    void *arg
);
static void peer_disc_incs(struct peer *peer);
#endif
...
static struct peer_dsc *peer_dsc_find(
    const struct peer_chr *chr, uint16_t dsc_handle, struct peer_dsc **out_prev
) {
    struct peer_dsc *dsc;

    struct peer_dsc *prev = peer_dsc_find_prev(chr, dsc_handle);
    if (prev == NULL) {
...
static int peer_dsc_add(
    struct peer *peer, uint16_t chr_val_handle, const struct ble_gatt_dsc *gatt_dsc
) {
    struct peer_dsc *prev;

    struct peer_svc *svc = peer_svc_find_range(peer, chr_val_handle);
    if (svc == NULL) {
    ...
    struct peer_chr *chr = peer_chr_find(svc, chr_val_handle, NULL);
    if (chr == NULL) {
    ...
    struct peer_dsc *dsc = peer_dsc_find(chr, gatt_dsc->handle, &prev);
    if (dsc != NULL) {
        ...
        SLIST_FOREACH(chr, &svc->chrs, next) {
            if (!chr_is_empty(svc, chr) && SLIST_EMPTY(&chr->dscs) &&
                ...
                int rc = ble_gattc_disc_all_dscs(
                    peer->conn_handle, chr->chr.val_handle, chr_end_handle(svc, chr), peer_dsc_disced, peer
                );
                if (rc != 0) {
...
uint16_t chr_end_handle(const struct peer_svc *svc, const struct peer_chr *chr) {
    const struct peer_chr *next_chr = SLIST_NEXT(chr, next);
    if (next_chr != NULL) {
...
struct peer_chr *peer_chr_find(
    ...
    struct peer_chr *prev = peer_chr_find_prev(svc, chr_val_handle);
    if (prev == NULL) {
...
static int peer_chr_add(
    struct peer *peer, uint16_t svc_start_handle, const struct ble_gatt_chr *gatt_chr
) {
    struct peer_chr *prev;

    struct peer_svc *svc = peer_svc_find(peer, svc_start_handle, NULL);
    if (svc == NULL) {
    ...
    struct peer_chr *chr = peer_chr_find(svc, gatt_chr->def_handle, &prev);
    if (chr != NULL) {
    ...
    SLIST_FOREACH(svc, &peer->svcs, next) {
        if (!peer_svc_is_empty(svc) && SLIST_EMPTY(&svc->chrs)) {
            ...
            int rc = ble_gattc_disc_all_chrs(
                peer->conn_handle, svc->svc.start_handle, svc->svc.end_handle, peer_chr_disced, peer
            );
            if (rc != 0) {
                peer_disc_complete(peer, rc);
            }
...
#if (MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) || MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES))
static struct peer_incl_svc *peer_incl_svc_find_prev(
    const struct peer_svc *svc, uint16_t incl_svc_handle
) {
...
static struct peer_incl_svc *peer_incl_svc_find(
    const struct peer_svc *svc, uint16_t incl_svc_handle, struct peer_incl_svc **out_prev
) {
    struct peer_incl_svc *prev;
    struct peer_incl_svc *incl_svc;
...
static void peer_incl_svc_delete(struct peer_incl_svc *incl_svc) {
    os_memblock_put(&peer_incl_svc_pool, incl_svc);
}

static int peer_inc_add(
    struct peer *peer, uint16_t svc_start_handle, const struct ble_gatt_incl_svc *gatt_incl_svc
) {
    struct peer_incl_svc *incl_svc;
...
static void peer_disc_incs(struct peer *peer) {
    ...
    if (svc != NULL && !peer_svc_is_empty(svc)) {
        rc = ble_gattc_find_inc_svcs(
            peer->conn_handle, svc->svc.start_handle, svc->svc.end_handle, peer_inc_disced, peer
        );
        if (rc != 0) {
            peer_disc_chrs(peer);
        }
...
static struct peer_svc *peer_svc_find(
    ...
    struct peer_svc *prev = peer_svc_find_prev(peer, svc_start_handle);
    if (prev == NULL) {
...
const struct peer_chr *peer_chr_find_uuid(
    const struct peer *peer, const ble_uuid_t *svc_uuid, const ble_uuid_t *chr_uuid
) {
    const struct peer_chr *chr;

    const struct peer_svc *svc = peer_svc_find_uuid(peer, svc_uuid);
    if (svc == NULL) {
        return NULL;
    }
...
const struct peer_dsc *peer_dsc_find_uuid(
...
static int peer_svc_add(struct peer *peer, const struct ble_gatt_svc *gatt_svc) {
    ...
    struct peer_svc *svc = peer_svc_find(peer, gatt_svc->start_handle, &prev);
    if (svc != NULL) {
...
int peer_disc_svc_by_uuid(
    uint16_t conn_handle, const ble_uuid_t *uuid, peer_disc_fn *disc_cb, void *disc_cb_arg
) {
    struct peer_svc *svc;
    ...
    int rc = ble_gattc_disc_svc_by_uuid(conn_handle, uuid, peer_svc_disced, peer);
    if (rc != 0) {
        return rc;
    }
...
int peer_disc_all(uint16_t conn_handle, peer_disc_fn *disc_cb, void *disc_cb_arg) {
    ...
    int rc = ble_gattc_disc_all_svcs(conn_handle, peer_svc_disced, peer);
    if (rc != 0) {
        return rc;
    }
...
int peer_delete(uint16_t conn_handle) {
    ...
    int rc = os_memblock_put(&peer_pool, peer);
    if (rc != 0) {
        return BLE_HS_EOS;
    }
...
#endif
{
    ...
    rc = os_mempool_init(
        &peer_svc_pool, max_svcs, sizeof(struct peer_svc), peer_svc_mem, "peer_svc_pool"
    );
    if (rc != 0) {
        rc = BLE_HS_EOS;
        goto err;
...
#if MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) || MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES)
    peer_incl_svc_mem = malloc(OS_MEMPOOL_BYTES(max_incl_svcs, sizeof(struct peer_incl_svc)));
    ...
    rc = os_mempool_init(
        &peer_chr_pool, max_chrs, sizeof(struct peer_chr), peer_chr_mem, "peer_chr_pool"
    );
    if (rc != 0) {
        rc = BLE_HS_EOS;
        goto err;
    ...
    rc = os_mempool_init(
        &peer_dsc_pool, max_dscs, sizeof(struct peer_dsc), peer_dsc_mem, "peer_dsc_pool"
    );
    if (rc != 0) {
        rc = BLE_HS_EOS;
        goto err;
